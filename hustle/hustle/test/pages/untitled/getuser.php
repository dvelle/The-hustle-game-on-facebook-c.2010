...
if ($num>0) {
	...
	if ($maxPage>1 AND $type=='nav') {
		...
		//Get all the rows
		$query="SELECT * FROM $tablename WHERE categoryid = '2' ORDER BY 'id' LIMIT $offset, $RPP";
		...
		while($row = mysql_fetch_array($result)) {
			...
			if ($row['gameid'] = $sresult_ar['gamename']){
					$champ = $sresult_ar['username'];					
					$score = $sresult_ar['score'];
				}else{
					$champ = "None";
					$score = 0;
				}
			 ...
			 echo "<td>" . $row['shortname'] . "<br />"."<a href=gamescreen.php?game=$file&amp;width=$width&amp;height=$height>PLAY</a>"."</td>"; 
			 echo "<td>" . "<b>Champion: </b>" . $champ ."<br />" . "High Score: ". $score . "</td";
			 echo "</tr>";
		}
