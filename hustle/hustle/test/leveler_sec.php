...
function lvl_chk($user, $lvl){
	...
	if($cur_lvl < $lvl){
		...
		$energy_max = getStat('epr',$userID);
  		$energy_max = $energy_max + $new;
  		setStat('epr',$userID,$energy_max);
		$change = 1;
		ch_lvl($user,$lvl,$change);
	} elseif($cur_lvl > $lvl){
		//downgraded
		$energy_max = getStat('epr',$userID);
  		$energy_max = $energy_max - $new;
...
function ch_lvl($user,$lvl,$change){
	$query = sprintf("UPDATE h_users SET cur_level = ('%s'),upgrade_flag = ('%s') WHERE UPPER(user) = UPPER('%s')",
		mysql_real_escape_string($lvl),
